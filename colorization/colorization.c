...
static float* luminanceRemapping(float* lab_s,
  float* deviation_s, float* deviation_t, float* means_s, float* means_t,
  int cols_t, int rows_t)
{
  float* lum = malloc(sizeof(float) * cols_t * rows_t * 3);

  for (int i = 0; i < rows_t; ++i)
    for (int j = 0; j < cols_t; ++j)
      for (int k = 0; k < 3; ++k)
      {
        lum[i * 3 * cols_t + j * 3 + k] = deviation_t[k] / deviation_s[k] *
               (lab_s[i * 3 * cols_t + j * 3 + k] - means_s[k]) + means_t[k];
      }
...
static void process(char *ims, char *imt, char* imd)
{
  ...
  int cols_s = pnm_get_width(im_s);
  int rows_s = pnm_get_height(im_s);

  int cols_t = pnm_get_width(im_t);
  int rows_t = pnm_get_height(im_t);

  unsigned short* img_s = pnm_get_image(im_s);
  unsigned short* img_t = pnm_get_image(im_t);

  float* lms_s = rgb2lms(img_s, cols_s, rows_s);
  float* lms_t = rgb2lms(img_t, cols_t, rows_t);
  ...
  // Deviation
  float* deviation_s = standardDeviations(means_s, lab_s, cols_s, rows_s);
  float* deviation_t = standardDeviations(means_t, lab_t, cols_t, rows_t);

  float* lum = luminanceRemapping(lab_s, deviation_s, deviation_t,
                                  means_s, means_t,
                                  cols_t, rows_t);

  float* lms_d = lab2lms(lum, cols_t, rows_t);
  unsigned short* rgb = lms2rgb(lms_d, cols_t, rows_t);

  pnm im_d = pnm_new(cols_t, rows_t, PnmRawPpm);
  unsigned short* buffer = pnm_get_image(im_d);
  fillBuffer(buffer, rgb, cols_t, rows_t);
